...
## Overview
Apply fixes for SQL performance issues identified by `sql-query-diagnosis` on `${AZURE_SQL_DATABASE}` (server `${AZURE_SQL_SERVER_FQDN}`). This skill handles risk assessment, human approval, and execution of the fix.
