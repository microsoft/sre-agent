...
## Overview
Resolve SQL blocking chains on `${AZURE_SQL_DATABASE}` (server `${AZURE_SQL_SERVER_FQDN}`) by killing the head blocker session, after risk assessment and human approval.
