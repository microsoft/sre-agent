...
# Force UTF-8 to avoid Windows cp1252 UnicodeEncodeError in `az acr build` streaming output
...
# ── Write .deployed/<lab>.json for the meta-sim and multi-lab launcher ──────
try {
    $labsRoot = Resolve-Path "$PSScriptRoot\..\.." -ErrorAction Stop
    $deployedDir = Join-Path $labsRoot ".deployed"
    if (-not (Test-Path $deployedDir)) { New-Item -ItemType Directory -Path $deployedDir -Force | Out-Null }
    $rec = [ordered]@{
        name           = "zava-athletic"
        deployedAt     = (Get-Date).ToUniversalTime().ToString("o")
        subscriptionId = (try { Get-AzdValue "AZURE_SUBSCRIPTION_ID" } catch { "" })
        resourceGroup  = $RG
        region         = (try { Get-AzdValue "AZURE_LOCATION" } catch { "" })
        sreAgentName   = $agentName
        portalUrl      = "http://$ingressIP/"
        labConfigPath  = ""
    }
    $json = $rec | ConvertTo-Json -Depth 5
    Set-Content -Path (Join-Path $deployedDir "zava-athletic.json") -Value $json -Encoding UTF8
    Write-Host "✓ Recorded deployment in labs/.deployed/zava-athletic.json" -ForegroundColor Green
} catch {
    Write-Host "WARN: could not write .deployed/zava-athletic.json — $_" -ForegroundColor Yellow
}
