...
| `services` | `["{{AZ_APP_PREFIX}}-outage","{{AZ_APP_PREFIX}}-meter","{{AZ_APP_PREFIX}}-grid","{{AZ_APP_PREFIX}}-notify","{{AZ_APP_PREFIX}}-portal"]` | MUST use the {{AZ_APP_PREFIX}}-* short names because that's what `ContainerAppName_s` contains in this workspace. |
...
|---|---|
| {{AZ_APP_PREFIX}}-outage | outage-api |
| {{AZ_APP_PREFIX}}-meter  | meter-api |
| {{AZ_APP_PREFIX}}-grid   | grid-status-api |
| {{AZ_APP_PREFIX}}-notify | notification-svc |
| {{AZ_APP_PREFIX}}-portal | portal-web |
...
`'{{AZ_APP_PREFIX}}-outage','{{AZ_APP_PREFIX}}-meter','{{AZ_APP_PREFIX}}-grid','{{AZ_APP_PREFIX}}-notify','{{AZ_APP_PREFIX}}-portal'`.
...
```
az containerapp show -n <name> -g {{AZ_RG}} \
