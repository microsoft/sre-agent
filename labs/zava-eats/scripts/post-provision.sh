...
# ── Step 5/5: Apply custom skill + python tool via srectl ────────────────────
# Exercises the full srectl path: PythonTool via apply-yaml, custom Skill via
# `skill apply`, and a non-interactive thread invocation via `thread new --wait`.
# Gated on srectl being on PATH AND not explicitly skipped.
echo ""
echo "🔧 Step 5/5: Applying custom skill + tool via srectl..."
if [ "${LABS_SKIP_SRECTL:-0}" = "1" ]; then
  echo "   ⏭️  Skipped (LABS_SKIP_SRECTL=1)"
elif ! command -v srectl >/dev/null 2>&1; then
  echo "   ⏭️  Skipped — srectl not on PATH"
  echo "      To enable: install srectl (private preview via aka.ms/sreagent-onboarding) and re-run:"
  echo "        cd $(dirname "$0")/.. && bash scripts/post-provision.sh --srectl-only"
else
  set +e
  SRECTL_DIR="$(cd "$(dirname "$0")/../sre-config" && pwd)"
  ( cd "$SRECTL_DIR" && srectl init --resource-url "$AGENT_ENDPOINT" ) >/tmp/srectl-init.log 2>&1
  RC=$?
  if [ $RC -ne 0 ]; then
    echo "   ⚠️  srectl init failed (rc=$RC). Output:"; sed 's/^/      /' /tmp/srectl-init.log | head -20
  else
    # 1) Tool via apply-yaml
    TOOL_YAML="$SRECTL_DIR/tools/CheckGrubifyHealth/CheckGrubifyHealth.yaml"
    if [ -f "$TOOL_YAML" ]; then
      echo "   • Applying tool: CheckGrubifyHealth"
      ( cd "$SRECTL_DIR" && srectl apply-yaml --file "tools/CheckGrubifyHealth/CheckGrubifyHealth.yaml" ) >/tmp/srectl-tool.log 2>&1
      RC=$?
      [ $RC -eq 0 ] && echo "     ✅ applied" || { echo "     ⚠️  failed (rc=$RC):"; sed 's/^/        /' /tmp/srectl-tool.log | head -10; }
    fi

    # 2) Skill via skill apply
    if [ -f "$SRECTL_DIR/skills/grubify-diagnosis/SKILL.md" ]; then
      echo "   • Applying skill: grubify-diagnosis"
      ( cd "$SRECTL_DIR" && srectl skill apply --name grubify-diagnosis ) >/tmp/srectl-skill.log 2>&1
      RC=$?
      [ $RC -eq 0 ] && echo "     ✅ applied" || { echo "     ⚠️  failed (rc=$RC):"; sed 's/^/        /' /tmp/srectl-skill.log | head -10; }
    fi

    # 3) Smoke-test: fire-and-forget thread invocation (--no-wait works in non-TTY)
    echo ""
    echo "   🧵 Smoke-test: srectl thread new --no-wait (incident-handler)"
    PROMPT="Use the grubify-diagnosis skill and CheckGrubifyHealth tool to confirm Grubify health. Reply with status_code and a 1-line summary."
    ( cd "$SRECTL_DIR" && srectl thread new --agent incident-handler --message "$PROMPT" --no-wait ) >/tmp/srectl-thread.log 2>&1
    RC=$?
    if [ $RC -eq 0 ]; then
      THREAD_ID=$(grep -oE 'Thread ID: [a-f0-9-]+' /tmp/srectl-thread.log | awk '{print $3}' | head -1)
      echo "     ✅ message sent. Thread ID: ${THREAD_ID:-(see log)}"
      echo "        Follow live: https://sre.azure.com — open the agent and find this thread."
    else
      echo "     ⚠️  thread creation failed (rc=$RC):"; sed 's/^/        /' /tmp/srectl-thread.log | tail -15
    fi
  fi
  set -e
fi
...
# ── Record deployment for the Zava Unlimited meta-sim ───────────────────────
LABS_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
DEPLOYED_DIR="$LABS_ROOT/.deployed"
mkdir -p "$DEPLOYED_DIR"
LAB_NAME="zava-eats"
SUB_ID="$(az account show --query id -o tsv 2>/dev/null || echo '')"
RG_NAME="${AZURE_RESOURCE_GROUP:-${RESOURCE_GROUP:-}}"
REGION="${AZURE_LOCATION:-}"
cat > "$DEPLOYED_DIR/$LAB_NAME.json" <<EOF
{
  "name": "$LAB_NAME",
  ...
  "region": "$REGION",
  "sreAgentName": "${SRE_AGENT_NAME:-${AGENT_NAME:-}}",
  "portalUrl": "${AGENT_ENDPOINT:-${PORTAL_URL:-}}",
  "labConfigPath": ""
}
EOF
echo ""
echo "  Recorded deployment in labs/.deployed/$LAB_NAME.json"
