...
- **App**: Grubify food-ordering sample — Node.js / Express API + frontend
- **Compute**: Azure Container Apps (API + frontend, fronted by Azure Container Registry)
- **Data**: None (in-memory state in the API)
- **Observability**: Log Analytics + Application Insights + 1 Azure Monitor alert rule (HTTP 5xx → auto-triggers agent)
- **SRE Agent**: Single agent with managed identity, knowledge base, and 3 custom subagents — `incident-handler` (logs + KQL + runbook), `code-analyzer` (source-code root cause + GitHub issues), `issue-triager` (customer-issue triage). Response plan routes alerts; optional GitHub OAuth connector enables Scenarios 2 & 3; optional scheduled task runs issue triage every 12h. Global tools (DevOps + Python plotting) enabled.
- **Simulator**: Bash break/fix scripts (`scripts/break-app.sh`, `scripts/create-sample-issues.sh`) — Windows users invoke via Git Bash
- **CI/CD**: `azd up` + `scripts/setup.sh` one-command wrapper (login → register providers → provision → post-provision agent config)

## What it's about

The starter lab is the **on-ramp to Azure SRE Agent for first-time PMs/SREs/customers** — the simplest end-to-end `azd up` demo in this repo. It deploys the Grubify food-ordering sample app on Container Apps, an SRE Agent with three custom subagents, and the wiring (managed identity, RBAC, knowledge base, response plan, Azure Monitor alert) needed for the agent to autonomously investigate an HTTP 5xx incident.

The lab teaches three break/fix patterns layered by persona: **(1) IT Operations** — break the app, agent investigates logs + KQL + runbook and remediates (no GitHub required); **(2) Developer** — same break, agent additionally searches Grubify source code, finds the root-cause file:line, and opens a GitHub issue (requires forking Grubify); **(3) Workflow Automation** — agent triages incoming customer issues with classifications, labels, and comments via the scheduled task. Demo flow: `bash scripts/setup.sh` (~5–8 min provision + post-provision) → verify on `sre.azure.com` → run `bash scripts/break-app.sh` → ask the agent to investigate.
