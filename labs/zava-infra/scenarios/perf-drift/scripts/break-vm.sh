...
# Try azd env first, then fall back to env var, then to a default derived from azd env name
RESOURCE_GROUP="${RESOURCE_GROUP_NAME:-}"
if [[ -z "$RESOURCE_GROUP" ]] && command -v azd &>/dev/null; then
...
if [[ -z "$RESOURCE_GROUP" ]]; then
  echo "ERROR: RESOURCE_GROUP not set. Pass via RESOURCE_GROUP_NAME env var or run from azd env." >&2
  exit 1
fi
