...
- **App**: Node.js demo app (`app/server.js`) with an intentional latency bug — provides realistic incident telemetry
- **Compute**: Azure App Service (Linux) + Azure Logic App (auth bridge with Managed Identity that acquires an AAD token and authenticates the inbound webhook to the SRE Agent HTTP Trigger)
- **Data**: None
- **Observability**: Application Insights + Log Analytics
- **SRE Agent**: Existing SRE Agent instance (provisioned out-of-band) + custom skill `terraform-drift-analysis` (drift classification — Benign / Risky / Critical — and incident-correlated remediation guidance) + custom HTTP Trigger that the Logic App POSTs to
- **Simulator**: PowerShell scripts (`induce-drift.ps1` — 3 drift types; `simulate-tfc-notification.ps1` — fake TFC webhook; `generate-load.ps1` — produces App Insights latency data; `revert-drift.ps1` — cleanup)
- **CI/CD**: Terraform (≥ 1.5) — `terraform init && terraform apply` for infra, `scripts/deploy-app.ps1` for the demo app; no `azd` (deliberate, since the lab is a Terraform Cloud + IaC story)

## What it's about

This lab is for PMs, SREs, and platform engineers who want to see how the **Azure SRE Agent moves IaC operations from "diff-and-alert" to "diagnose-correlate-remediate"** for Terraform drift. A Terraform Cloud run notification (or a simulated webhook) is bridged into the agent via a Logic App + HTTP Trigger, and the agent autonomously investigates: what changed, who changed it, why (by correlating with App Insights latency and Activity Log), how risky the change is (Benign tags vs Risky TLS downgrade vs Critical SKU change), and whether it's safe to revert right now — including the smart "do NOT revert the SKU while the latency bug is still deployed" recommendation.

The lab teaches a single end-to-end pattern: webhook-driven IaC investigation with smart remediation, Teams notification, self-improving skill, and a follow-up PR. Demo flow: `terraform apply` → `deploy-app.ps1` → create the skill + HTTP Trigger in the SRE Agent UI (per `IMPLEMENTATION-GUIDE.md`) → `generate-load.ps1` → `induce-drift.ps1` → `simulate-tfc-notification.ps1` → watch the agent classify all 3 drift types, recommend the right remediation per severity, and ship a fix PR for the latency bug.
...
cd sre-agent/labs/zava-infra/scenarios/tf-drift/terraform
