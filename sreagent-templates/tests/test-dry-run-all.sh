...
for test in tests/test-connector-v2.sh tests/test-model-provider.sh tests/test-export-prerequisites.sh tests/test-region-discovery.sh tests/test-supported-regions.py; do
  name=$(basename "$test")
  echo "════════════ $name ════════════"
  if [[ "$test" == *.py ]]; then
