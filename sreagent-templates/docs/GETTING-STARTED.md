...
The machine-readable list in [supported-regions.json](../supported-regions.json) is validated against every deployment template and recipe. See the [Azure SRE Agent supported-regions reference](https://learn.microsoft.com/azure/sre-agent/supported-regions) for service updates.
