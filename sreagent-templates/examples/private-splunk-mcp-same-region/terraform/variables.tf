variable "resource_group_name" {
  ...
  type        = string
  default     = "rg-private-splunk-same-region"
...
variable "location" {
  description = "Azure region for the existing SRE Agent, shared VNet, and private Splunk VM."
  type        = string
  default     = "eastus2"
...
variable "vnet_address_prefix" {
  description = "IPv4 /16 address space for the shared lab VNet."
  type        = string
  default     = "10.100.0.0/16"

  validation {
    condition     = can(cidrhost(var.vnet_address_prefix, 0)) && can(regex("^([0-9]{1,3}\\.){3}[0-9]{1,3}/16$", var.vnet_address_prefix))
    error_message = "vnet_address_prefix must be a valid IPv4 /16 CIDR."
  }
}
