locals {
  agent_vnet_name        = "${var.name_prefix}-agent-vnet"
  splunk_vnet_name       = "${var.name_prefix}-splunk-vnet"
  ...
  splunk_vnet_key        = try(join(".", slice(split(".", cidrhost(var.splunk_vnet_address_prefix, 0)), 0, 2)), "")
...
resource "azurerm_resource_group" "lab" {
  ...
  lifecycle {
    precondition {
      condition     = lower(var.agent_location) != lower(var.splunk_location)
      error_message = "agent_location and splunk_location must be different to exercise cross-region connectivity."
    }

    precondition {
      condition     = local.agent_vnet_key != local.splunk_vnet_key
      error_message = "agent_vnet_address_prefix and splunk_vnet_address_prefix must not overlap."
    }

    precondition {
      condition     = local.agent_vnet_key == local.agent_subnet_vnet_key
      error_message = "agent_subnet_prefix must be fully contained in agent_vnet_address_prefix."
    }

    precondition {
      condition     = local.splunk_vnet_key == local.splunk_subnet_vnet_key
      error_message = "splunk_subnet_prefix must be fully contained in splunk_vnet_address_prefix."
    }
